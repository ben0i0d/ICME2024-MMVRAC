...
if __name__ == "__main__":
    ...
    parser.add_argument('--shift-t-dir', default=None)
    ...
    if 'UCLA' in arg.dataset:
        ...
        npz_data = np.load('./data/' + 'uav/' + 'y_test.npz')
        label =np.where(npz_data['y_test'] > 0)[1]
    else:
    ...
    if arg.shift_t_dir is not None:
        with open(os.path.join(arg.shift_t_dir, 'epoch1_test_score.pkl'), 'rb') as r7:
    ...
    for i in tqdm(range(len(label))):
        ...
        _, r66 = r6[i]
        #_, r77 = r7[i]
        r =0.5*r44+0.9*r33+r11+0.35*r22+0.45*r55+0.3*r66#0.5*r44+0.9*r33+r11+0.35*r22+0.45*r55  #+ r44 * arg.alpha[2] #+ r44 * arg.alpha[3]+ r22 * arg.alpha[1]
        rank_5 = r.argsort()[-5:]
