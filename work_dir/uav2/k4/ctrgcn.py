...
class scale_tcn(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size=3, stride=1):
        super(scale_tcn, self).__init__()
        pad = int((kernel_size - 1) / 2)
        self.conv = nn.Conv2d(in_channels,out_channels,kernel_size=1)
       # self.conv2 = nn.Conv2d(in_channels,512,kernel_size=1)
        self.bn = nn.BatchNorm2d(out_channels)
        self.bn1 = nn.BatchNorm2d(2*out_channels)
        self.relu = nn.ReLU(inplace=True)
        conv_init(self.conv)
        bn_init(self.bn, 1)
        self.conv1=nn.Conv2d(out_channels, 2*out_channels, kernel_size=(kernel_size, 1), padding=(pad, 0),
                              stride=(stride, 1))
        self.fc=nn.Linear(2*out_channels,out_channels)
        self.fc1=nn.Linear(out_channels,6)
    def forward(self, x):
        N,C,T,V=x.size()
...
class cbam_block(nn.Module):
    ...
    def __init__(self, channel,outputs,stride=2, ratio=8, kernel_size=1):
        ...
        self.conv = nn.Conv2d(channel, outputs, kernel_size=(3, 1), padding=(1, 0),
                              stride=(stride, 1))
...
class Model(nn.Module):
    def __init__(self, num_class=60, num_point=25, num_person=2, graph=None, graph_args=dict(), in_channels=3,k=4,
        ...
        self.l10 = TCN_GCN_unit(base_channel*4, base_channel*8, A, adaptive=adaptive)
    ...
    def forward(self, x):
        ...
        x1 = self.A_vector.expand(N*M*T, -1, -1).cuda(x.get_device()).float() 
        x=torch.matmul(x1,x)
