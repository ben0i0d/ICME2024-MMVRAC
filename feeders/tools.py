...
def temperal_crop(data_numpy, temperal_padding_ratio=3):
    C, T, V, M = data_numpy.shape
    padding_len = T // temperal_padding_ratio
    frame_start = np.random.randint(0, padding_len * 2 + 1)
    data_numpy = np.concatenate((data_numpy[:, :padding_len],
                                 data_numpy,
                                 data_numpy[:, -padding_len:]),
                                axis=1)
...
def random_rotate(seq):
    ...
    for axis in total_axis:
        if axis == main_axis:
            rotate_angle = random.uniform(0, 60)
    ...
    flip_prob  = random.random()
    if flip_prob < 0.5:
        sacle=random.randint(1, 100)
        new_seq=new_seq*sacle
    else:
        sacle=random.uniform(0.01, 1)
        new_seq=new_seq*sacle
    return new_seq
...
def scale_change(data_numpy):
    C, T, V, M = data_numpy.shape
    dis=data_numpy[:,:,11:12,:]-data_numpy[:,:,12:13,:]
    dis1=torch.squeeze(torch.pow(dis,2),axis=2)
    dis2=torch.mean(dis1,dim=2)
   
    dis3=torch.sum(dis2,dim=0)
    dis4=torch.sqrt(dis3)
    dis5=torch.mean(dis4)
    dis6=dis5+0.0001
    disa=100/dis6
    
    print(disa)
    data_numpy=data_numpy*disa
    return data_numpy
